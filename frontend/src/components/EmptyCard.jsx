...
const EmptyCard = ({ imgSrc, message, setOpenAddEditModal }) => {
  return (
    <div className='flex flex-col items-center justify-center mt-16 mx-auto p-8 bg-gray-100 rounded-lg shadow-lg border border-gray-300 max-w-sm'>
      ...
      <p className='text-[16px] font-semibold text-gray-800 text-center mt-5'>{message}</p>

      <button className='mt-5 px-5 py-2 text-white text-sm font-medium bg-pink-400 rounded-full shadow-md hover:bg-pink-600 transition-all' onClick={() => {setOpenAddEditModal({ isShown: true, type: "add", data: null })}}>Create New</button>
    </div>
  )
}
